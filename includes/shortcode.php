...
function custom_add_ons(){
	...
	$customer = rcp_get_customer_by_user_id( $user_id );//get customer by user id
    $productscount = 0;
    ...
    $productatt = $product->get_attribute( 'RCP_membership' );//get RCP membership ID
    $show_prod = $product->get_attribute( 'show_related' );//get RCP membership ID
		if($customer!=false){//check if logged in
			$membership = $customer->get_memberships(//get the user membership
				array(
					'status'	=>	'active',
					'object_id' => $productatt //membership ID
				)
			);
		
			if(!empty($membership)){//check if user already bought the product, //dont show if already bought
				continue;
      }
